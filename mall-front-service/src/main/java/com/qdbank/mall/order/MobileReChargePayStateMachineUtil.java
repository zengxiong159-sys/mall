...
@Component
@Slf4j
public class MobileReChargePayStateMachineUtil {

    @Resource
    @Qualifier("mobileStateMachineFactory" )
    StateMachineFactory<MobileReChargePayOrderStatus, MobileReChargePayOrderEvent> stateMachineFactory;

    @Autowired
    private StateMachinePersister<MobileReChargePayOrderStatus, MobileReChargePayOrderEvent,OrderDO> persister;

    @Autowired
    OrderService orderService;


    public Message initMessage(MobileReChargePayOrderEvent event, Map<String,Object> params){
    ...
    public void execute(OrderDO order,Message<MobileReChargePayOrderEvent> message){
        if(order==null && message.getPayload()==MobileReChargePayOrderEvent.CREATE){
            order = new OrderDO();
        }
        //获取客户号
        if(message.getPayload()!=MobileReChargePayOrderEvent.TIMEOUT_CANCEL && message.getPayload()!= MobileReChargePayOrderEvent.NOTIC
                && message.getPayload()!= MobileReChargePayOrderEvent.CHARGE_NOTICE
        ){
        ...
        try{
            ...
            if(error.isHasError()){
                ...
                if(obj !=null && obj instanceof MobileReChargePayOrderStatus){
                    MobileReChargePayOrderStatus status = (MobileReChargePayOrderStatus) stateMachine.getState().getId();
                    if(status!=null && status.metricsHolder!=null){
                        if(MobileReChargePayOrderStatus.REFUND==status){
    ...
    //    StateMachine machine = stateMachineThreadLocal.get();
        ...
        if (null == machine){
            machine = stateMachineFactory.getStateMachine(Constant.MOBILE_RECHARGEPAY_ORDER_MACHINE);
        }
        try {
            machine.start();
            //订单状态初始化至状态机
