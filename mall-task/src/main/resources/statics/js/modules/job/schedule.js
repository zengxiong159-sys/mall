$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'task/sys/schedule/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		update: function () {
			...
			$.get(baseURL + "task/sys/schedule/info/"+jobId, function(r){
				vm.showList = false;
                vm.title = "修改";
		...
		saveOrUpdate: function (event) {
			var url = vm.schedule.jobId == null ? "task/sys/schedule/save" : "task/sys/schedule/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "task/sys/schedule/delete",
                    contentType: "application/json",
		...
		pause: function (event) {
			...
			confirm('确定要暂停选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "task/sys/schedule/pause",
                    contentType: "application/json",
				    data: JSON.stringify(jobIds),
				    success: function(r){
						if(r.code == 0){
		...
		resume: function (event) {
			...
			confirm('确定要恢复选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "task/sys/schedule/resume",
                    contentType: "application/json",
				    data: JSON.stringify(jobIds),
				    success: function(r){
						if(r.code == 0){
		...
		runOnce: function (event) {
			...
			confirm('确定要立即执行选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "task/sys/schedule/run",
                    contentType: "application/json",
				    data: JSON.stringify(jobIds),
				    success: function(r){
						if(r.code == 0){
